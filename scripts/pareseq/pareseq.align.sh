...
BOWTIE2INDEX=/grid/martienssen/data_norepl/bberube/maize/tpd/pareseq/H7WCTBGXG/index/tpd1asm202009.bowtie2
...
genomeCoverageBed -ibam $SGE_TASK_ID.all.bam -bga -5 -scale $NORM -strand + 2>>$LOG |
    sortBed -i - 2>>$LOG >$SGE_TASK_ID.P.bdg
genomeCoverageBed -ibam $SGE_TASK_ID.all.bam -bga -5 -scale $NORM -strand - 2>>$LOG |
    sortBed -i - 2>>$LOG >$SGE_TASK_ID.M.bdg
